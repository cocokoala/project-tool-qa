...
public class SvnUtil {
        ...
        public static Boolean checkOut() {
            ...
            try {
                repositoryURL = SVNURL.parseURIEncoded(SvnConfig.SvnPath);
            ...
            // 实例化客户端管理类
            ourClientManager = SVNClientManager.newInstance((DefaultSVNOptions) options, SvnConfig.SvnUserName, SvnConfig.SvnPassWord);

            // 要把版本库的内容check out到的目录
            File wcDir = new File(SvnConfig.TargetPath);
        ...
        public static int doUpdate() {
            ...
            try {
                ...
                ourClientManager = SVNClientManager.newInstance((DefaultSVNOptions) options, SvnConfig.SvnUserName, SvnConfig.SvnPassWord);
                // 要更新的文件
                File updateFile = new File(SvnConfig.TargetPath);
                // 获得updateClient的实例
                SVNUpdateClient updateClient = ourClientManager.getUpdateClient();
                updateClient.setIgnoreExternals(false);
                // 执行更新操作
        ...
        public static Boolean doCommit(List<String> fileRelativePathList) {
            ...
            ourClientManager = SVNClientManager.newInstance((DefaultSVNOptions) options, SvnConfig.SvnUserName, SvnConfig.SvnPassWord);
            // 要提交的文件夹子
            File commitFile = new File(SvnConfig.TargetPath);
            ...
            try {
                if (fileRelativePathList != null && fileRelativePathList.size() > 0) {
                    for (int i = 0; i < fileRelativePathList.size(); i++) {
                        ...
                        addFile = new File(SvnConfig.TargetPath + "/" + strPath);
        ...
        public static Boolean doCommit(String fileRelativePath) {
            ...
            try {
                if (fileRelativePath != null && fileRelativePath.trim().length() > 0) {
                    addFile = new File(SvnConfig.TargetPath + "/" + fileRelativePath);
                    status = ourClientManager.getStatusClient().doStatus(addFile, true);
                    // 如果此文件是新增加的则先把此文件添加到版本库，然后提交。
                    if (null == status || status.getContentsStatus() == SVNStatusType.STATUS_UNVERSIONED) {
                        // 把此文件增加到版本库中
